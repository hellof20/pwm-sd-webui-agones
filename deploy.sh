...
helm install sd-agones-release --namespace agones-system -f agones/values.yaml agones/agones --version 1.30.0
gcloud compute --project=${PROJECT_ID} firewall-rules create agones-sd-firewall \
--network=${VPC_NETWORK} \
--action=ALLOW \
...
cd sd-webui
DOCKER_BUILDKIT=1 docker build . -t $SD_WEBUI_IMAGE
